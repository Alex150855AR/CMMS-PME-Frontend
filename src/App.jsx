    ...
    <div className="space-y-6 animate-in fade-in">
      <div className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 items-end">
        <div className="space-y-1">
          <label className="text-[10px] font-black text-slate-400 uppercase">Filtrar Proyecto</label>
          <select className="w-full border rounded-xl p-2.5 text-sm font-bold" value={projectFilter} onChange={e => setProjectFilter(e.target.value)}>
            <option value="">-- Todos --</option>
            {uniqueProjects.map(p => <option key={p} value={p}>{p}</option>)}
          </select>
        </div>
        <div className="space-y-1">
          <label className="text-[10px] font-black text-slate-400 uppercase">Filtrar Modelo</label>
          <select className="w-full border rounded-xl p-2.5 text-sm font-bold" value={modelFilter} onChange={e => setModelFilter(e.target.value)}>
            <option value="">-- Todos --</option>
            {availableModels.map(m => <option key={m} value={m}>{m}</option>)}
          </select>
        </div>
        <div className="space-y-1">
          <label className="text-[10px] font-black text-slate-400 uppercase">Buscar Serial</label>
          <div className="relative">
            <Search className="absolute left-3 top-2.5 w-4 h-4 text-slate-400" />
            <input className="pl-9 border rounded-xl p-2.5 text-sm w-full" placeholder="..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
          </div>
        </div>
        <button onClick={() => { setIsEditing(false); setFormAsset(initialAsset); setActiveModal('asset'); }} className="bg-blue-600 text-white px-6 py-2.5 rounded-xl font-bold flex items-center justify-center hover:bg-blue-700 active:scale-95"><Plus className="w-5 h-5 mr-2"/> Nuevo Activo</button>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredAssets.map(a => (
           <div key={a.asset_id} className="bg-white rounded-2xl shadow-sm border border-slate-200 p-5 relative hover:shadow-md group">
              ...
              <tr key={ot.wo_id} className="hover:bg-blue-50/10 transition-colors">
                ...
                <td className="px-5 py-5 text-center">
                   <span className={`px-2 py-1 rounded-full font-bold text-[10px] uppercase border ${
                     ot.status === 'Programado' ? 'bg-gray-100 text-black border-gray-300' :
                     ot.status === 'Reprogramado' ? 'bg-yellow-100 text-yellow-800 border-yellow-200' :
                     ot.status === 'Completado' ? 'bg-green-100 text-green-800 border-green-200' :
                     ot.status === 'No Completado' ? 'bg-red-100 text-red-800 border-red-200' :
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {dbStaff.filter(s => s.role !== 'Administrador').map(s => (
          <div key={s.user_id} className="bg-white p-6 rounded-3xl shadow-sm border border-slate-200 flex flex-col relative group hover:shadow-md">
            ...
            {/* BOTONES DE GESTIÓN TÉCNICA */}
            <div className="grid grid-cols-2 gap-3 mb-4">
               <button onClick={() => openTechHistory(s, 'pending')} className="flex items-center justify-center bg-orange-50 text-orange-600 text-xs font-bold py-2 rounded-xl hover:bg-orange-100 transition-colors border border-orange-200">
                  <ListTodo className="w-3 h-3 mr-2"/> Pendientes ({s.active_tasks || 0})
               </button>
               <button onClick={() => openTechHistory(s, 'all')} className="flex items-center justify-center bg-blue-50 text-blue-600 text-xs font-bold py-2 rounded-xl hover:bg-blue-100 transition-colors border border-blue-200">
                  <History className="w-3 h-3 mr-2"/> Historial
               </button>
            </div>
    ...
    <div className="bg-slate-50 font-sans text-slate-800 h-screen flex overflow-hidden">
      ...
      {activeModal === 'asset' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-xl p-6 sm:p-10 my-4 sm:my-8 max-h-[calc(100vh-2rem)] overflow-y-auto">
            <h3 className="text-2xl font-black text-slate-800 mb-6 uppercase tracking-tighter border-b border-slate-50 pb-4">{isEditing ? 'Actualizar Activo' : 'Nuevo Activo'}</h3>
            <form onSubmit={handleSaveAsset} className="space-y-4">
               <div className="grid grid-cols-2 gap-4">
                 <div><label className="text-[11px] font-black text-blue-600 uppercase">Proyecto</label>
                  <input list="projects" required className="w-full bg-slate-50 border rounded-2xl p-3 text-sm font-bold" value={formAsset.project_name} onChange={e=>setFormAsset({...formAsset,project_name:e.target.value})}/>
                  <datalist id="projects">{uniqueProjects.map(p=><option key={p} value={p}/>)}</datalist>
                 </div>
                 <div><label className="text-[11px] font-black text-blue-600 uppercase">Modelo</label>
                  <input list="models" required className="w-full bg-slate-50 border rounded-2xl p-3 text-sm font-bold" value={formAsset.model_name} onChange={e=>setFormAsset({...formAsset,model_name:e.target.value})}/>
                  <datalist id="models">{availableModels.map(m=><option key={m} value={m}/>)}</datalist>
                 </div>
               </div>
               <div className="grid grid-cols-2 gap-4">
                 <div><label className="text-[11px] font-black text-slate-400 uppercase">Serial</label><input required className="w-full bg-slate-50 border rounded-2xl p-3 text-sm font-mono" value={formAsset.serial_number} onChange={e=>setFormAsset({...formAsset,serial_number:e.target.value})}/></div>
                 <div><label className="text-[11px] font-black text-slate-400 uppercase">Nombre</label><input required className="w-full bg-slate-50 border rounded-2xl p-3 text-sm font-bold" value={formAsset.fixture_name} onChange={e=>setFormAsset({...formAsset,fixture_name:e.target.value})}/></div>
               </div>
               <div className="grid grid-cols-2 gap-4">
                 <div><label className="text-[11px] font-black text-slate-400 uppercase">Línea</label><input required className="w-full bg-slate-50 border rounded-2xl p-3 text-sm" value={formAsset.production_line} onChange={e=>setFormAsset({...formAsset,production_line:e.target.value})}/></div>
                 <div><label className="text-[11px] font-black text-slate-400 uppercase">Estación</label><input required className="w-full bg-slate-50 border rounded-2xl p-3 text-sm" value={formAsset.station} onChange={e=>setFormAsset({...formAsset,station:e.target.value})}/></div>
               </div>
               <div><label className="text-[11px] font-black text-slate-400 uppercase">Estado</label><select className="w-full bg-slate-50 border rounded-2xl p-3 text-sm font-bold" value={formAsset.condition_status} onChange={e=>setFormAsset({...formAsset,condition_status:e.target.value})}><option>Activo</option><option>Inactivo</option><option>En Mantenimiento</option></select></div>
               <div className="grid grid-cols-2 gap-4">
                 <div><label className="text-[11px] font-black text-slate-400 uppercase">Foto (PNG)</label><input type="file" accept="image/png" className="w-full text-xs" onChange={e => setFormAsset({...formAsset, image_file: e.target.files[0]})}/></div>
                 <div><label className="text-[11px] font-black text-slate-400 uppercase">Doc (PDF)</label><input type="file" accept="application/pdf" className="w-full text-xs" onChange={e => setFormAsset({...formAsset, doc_file: e.target.files[0]})}/></div>
               </div>
               <div><label className="text-[11px] font-black text-slate-400 uppercase">Descripción</label><textarea className="w-full bg-slate-50 border rounded-2xl p-3 text-sm h-20" value={formAsset.description} onChange={e=>setFormAsset({...formAsset,description:e.target.value})}/></div>
               <div className="flex gap-2">
                 <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-2xl font-black uppercase">Guardar</button>
      ...
      {activeModal === 'wo' && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white rounded-[2rem] shadow-2xl w-full max-w-2xl p-6 my-8 max-h-[calc(100vh-2rem)] overflow-y-auto">
            <h3 className="text-2xl font-black text-slate-800 mb-6 uppercase tracking-tighter border-b pb-4">{isEditing ? 'Editar Orden' : 'Nueva Orden'}</h3>
            <form onSubmit={handleSaveWO} className="space-y-4">
               ...
               {!isEditing ? (
                 ...
                 <div className="bg-slate-100 p-4 rounded-3xl border border-slate-200">
                    <label className="text-[10px] font-black text-slate-500 uppercase block mb-1">Activo Vinculado (No editable)</label>
               ...
               <div className="grid grid-cols-2 gap-4">
                  <div><label className="text-[11px] font-black text-slate-400 uppercase">Ubicación</label><input className="w-full bg-slate-50 border rounded-2xl p-3 text-sm" value={formWO.location} onChange={e=>setFormWO({...formWO, location:e.target.value})}/></div>
                  <div><label className="text-[11px] font-black text-slate-400 uppercase">Tipo</label><select className="w-full bg-slate-50 border rounded-2xl p-3 text-sm" value={formWO.type} onChange={e=>setFormWO({...formWO, type:e.target.value})}><option>Correctivo</option><option>Preventivo</option></select></div>
               </div>
               <div className="grid grid-cols-2 gap-4">
               ...
               <div><label className="text-[11px] font-black text-slate-400 uppercase">Técnico</label><select className="w-full bg-slate-50 border rounded-2xl p-3 text-sm" value={formWO.tech_id} onChange={e=>setFormWO({...formWO, tech_id:e.target.value})}><option value="">-- Seleccionar --</option>{dbStaff.map(s=><option key={s.user_id} value={s.user_id}>{s.full_name}</option>)}</select></div>
               
               <div className="flex gap-2">
                 <button type="submit" className="w-full bg-blue-600 text-white py-4 rounded-3xl font-black uppercase shadow-xl">Confirmar</button>
                 <button type="button" onClick={()=>setActiveModal(null)} className="w-full bg-slate-200 text-slate-500 py-4 rounded-3xl font-black uppercase">Cancelar</button>
               </div>
            </form>
          </div>
        </div>
      ...
      {/* --- NUEVO: MODAL HISTORIAL TÉCNICO --- */}
      {techHistoryModal.show && (
         <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/80 backdrop-blur-sm p-4 overflow-y-auto">
            <div className="bg-white rounded-[2.5rem] shadow-2xl w-full max-w-4xl p-8 max-h-[90vh] overflow-y-auto">
                <div className="flex justify-between items-center mb-6">
                    <div>
                        <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tighter">
                            {techHistoryModal.type === 'pending' ? 'ACTIVIDADES PENDIENTES' : 'HISTORIAL COMPLETO'}
                        </h2>
                        <p className="text-slate-400 font-bold">{techHistoryModal.tech?.full_name} ({techHistoryModal.tech?.employee_number})</p>
                    </div>
                    <button onClick={closeTechHistory} className="p-3 bg-slate-100 rounded-full hover:bg-red-50 hover:text-red-500 transition-colors"><X/></button>
                </div>
                                        ...
                                        <tr key={ot.wo_id} className="hover:bg-blue-50/20">
                                            ...
                                            <td className="p-4">
                                                {techHistoryModal.type === 'pending' ? (
                                                    <select 
                                                        className="border rounded p-2 text-xs font-bold text-slate-700 bg-white outline-none focus:ring-2 focus:ring-blue-500"
                                                        value={ot.status}
                                                        onChange={(e) => handleUpdateStatusFromTable(ot.wo_id, e.target.value)}
                                                    >
